...
USE [Utilities];
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('adt.qy_GetHumanaObs'))
   DROP PROC [adt].[qy_GetHumanaObs]
GO
CREATE PROCEDURE [adt].[qy_GetHumanaObs]
/* -----------------------------------------------------------------------------------------------------------
   Procedure Name  :  qy_GetHumanaObs
   Business Analyis:
   Project/Process :   
   Description     :  Get all rows from Humana Obs table.
...
--  Initialize Batch
 ...
 ----------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    SELECT 
	   ...
	   ,[CaseType]
	   ,[AuthId]
       ,CONVERT([nvarchar] (50), [FirstDay], 121) AS [FirstDay]
	   ,[SubscriberId]
	   ,[FirstName]
	   ,[LastName]
       ,CONVERT([nvarchar] (50), [DateOfBirth], 121) AS [DateOfBirth]
	   ,[GrouperId]
	   ,[PcpId]
	   ,[FaxTaxId]
	   ,[AuthType]
       ...
       ,CONVERT([nvarchar] (50), [LastDay], 121) AS [LastDay]
	   ,[DiagCode1]
	   ,[DiagDesc1]
	   ,[DiagCode2]
	   ,[DiagDesc2]
	   ,[DiagCode3]
	   ,[DiagDesc3]
	   ,CONVERT([nvarchar] (10), [ProcCode]) AS [ProcCode]
	   ,[ProcDesc]
	   ,[Gender]
       ,[SourceFullFilename]
    FROM [SimplifyVbcAdt8].[dbo].[HumanaObs];
