...
USE [Utilities];
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('adt.qy_GetHumanaDischarges'))
   DROP PROC [adt].[qy_GetHumanaDischarges]
GO
CREATE PROCEDURE [adt].[qy_GetHumanaDischarges]
/* -----------------------------------------------------------------------------------------------------------
   Procedure Name  :  qy_GetHumanaDischarges
   Business Analyis:
   Project/Process :   
   Description     :  Get all Discharges rows from Humana table.
...
--  Initialize Batch
 ...
 ----------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    SELECT 
       ...
       ,[SourceFullFilename] 
    FROM [SimplifyVbcAdt8].[dbo].[Humana]
    WHERE UPPER([AdmitOrDischarge]) = 'DISCHARGE';
