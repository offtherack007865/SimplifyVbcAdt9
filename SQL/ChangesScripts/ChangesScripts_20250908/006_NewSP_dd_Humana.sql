...
USE [Utilities];
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('adt.dd_Humana'))
   DROP PROC [adt].[dd_Humana]
GO
CREATE PROCEDURE [adt].[dd_Humana]
/* -----------------------------------------------------------------------------------------------------------
   Procedure Name  :  dd_Humana
   Business Analyis:
   Project/Process :   
   Description     :  Truncate Staging Humana table.
...
--  Initialize Batch
 ...
 ----------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    SET @BatchDetailDescription = '010/020:  Truncate Staging Humana'
    EXEC Admin.Utilities.logs.di_Batch @BatchOutID OUTPUT,  @BatchOutID, 'DetailStart', @BatchDetailDescription
	
	  SELECT @AnticipatedRecordCount = COUNT(*)
	                                   FROM [Staging].[adt].[Humana];
	  
    -- Truncate Staging Humana
    TRUNCATE TABLE [Staging].[adt].[Humana];
